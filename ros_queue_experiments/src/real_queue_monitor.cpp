...
void RealQueueMonitor::queueStatsCallback(const ros_queue_msgs::QueueServerStats::ConstPtr& msg)
{
    ...
    for (auto queue_stats : msg->queue_stats)
    {
        if(queue_stats.queue_name == queue_name_)
        {
            ...
            metric_performance_msg.queue_server_arrival_mean = queue_stats.arrival_mean;
            metric_performance_msg.target_value = queue_stats.departure_time_average;
            
            metric_performance_msg.real_continuous_average_diff_with_target = metric_performance_msg.queue_server_time_average_value - metric_performance_msg.target_value;
