...
VirtualQueueMonitor::VirtualQueueMonitor(ros::NodeHandle& nh): nhp_(nh), nh_(ros::NodeHandle()), localization_monitor_(nhp_), temperature_monitor_(nhp_), low_temperature_monitor_(nhp_), penalty_monitor_(nhp_)
{
...
void VirtualQueueMonitor::realStateCallback(const ros_queue_experiments::AuvStates::ConstPtr& msg)
{
    ...
    if(queue_server_stats_client_.call(queue_stats_metric_srv))
    {
        ...
        penalty_monitor_.realStateMetricCallback(msg, queue_stats_metric_srv);  
